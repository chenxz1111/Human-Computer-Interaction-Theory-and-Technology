...
/// </summary>
public abstract class GestureWidgetMove1D : Sensor
{
    ...
    public GestureWidgetMove1D(Handedness handedness = Handedness.Right)
    {
    ...
    public void GestureEventTrigger()
    {
        if (HandJointUtils.TryGetJointPose(TrackedHandJoint.Palm, _handedness, out var palmPose))
        {
            ...
            {
                if (GestureCondition(oldPalmPos, palmPose.Position))
                {
                    SensorTrigger();
                }
                else
